...
-- Global offset: pushes the entire map far away from the lobby (which sits near origin)
...
-- Spawn point inside Eco Lab (on the platform, facing the tower)
...
-- Disable other spawns so players always start in Eco Lab (only this map is active right now)
...
print("[gh-sync] EcoLabMap_v3 loaded - offset " .. tostring(MAP_OFFSET) .. ", spawn active")